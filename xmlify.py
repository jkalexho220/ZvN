...
files = ['zshared.c', 'upgrades.c', 'events.c', 'abilities.c', 'attackSteps.c',
 		'setup.c', 'database.c', 'cinematic.c', 'abilitiesHelpers.c', 'gameplayHelpers.c', 'zenoAbilities.c', 'nickAbilities.c', 'respawn.c', 'gameplay.c']
				...
				while line:
					...
					if not line.isspace():
						if not ERRORED:
							...
							templine = templine.replace('=', ' = ').replace(' =  = ', ' == ').replace('! = ', ' != ').replace(' >  = ', ' >= ').replace(' <  = ', '<=').replace('minInterval ', 'minInterval').replace('maxInterval ', 'maxInterval').replace('\t', ' ').replace(' /  * ', ' /* ').replace(' *  / ', ' */ ').replace(' +  + ', ' ++ ').replace(' -  - ', ' -- ').replace(' main ( void )', ' main ( )')
						...
						if not comment:
							if (len(templine) > 240):
								...
								print("Line " + str(ln) + ":\n    " + line)
							if len(templine) > 0 and NEED_SEMICOLON and not ERRORED and not (templine[-2:] == '*/') and not  (templine[-1] == ';' or templine[-1] == '}' or templine[-1] == '{' or templine[-1] == ','):
